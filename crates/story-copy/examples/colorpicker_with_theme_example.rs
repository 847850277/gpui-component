...
use story_copy::clipboard::ClipboardStory;
use story_copy::color_picker::ColorPickerStory;
...
fn main() {
    ...
    app.run(|cx: &mut App| {
        ...
        let title = "ColorPicker Example";
        let title = SharedString::from(title.to_string());
        cx.open_window(option, |window, cx| {
            // 创建视图
            let view = ColorPickerStory::view(window, cx);
